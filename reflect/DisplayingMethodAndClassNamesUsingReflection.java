...
public class DisplayingMethodAndClassNamesUsingReflection {
    public static void main(String args[]) {
        ...
        s = findAllClassesUsingClassLoader("reflect");
                ...
                for (Class y : s) {
                    if (x.equals(y)) {
                        ...
                        for (Method method2 : methodsB) {
                            if ((method.getName()).equals(method2.getName()) && !(commonMethods.contains(method))
                                    && !(commonMethods.contains(method2))) {
                                ...
                                if (checkMethodAnnotations(method, method2)) {
                                    if (checkParameterAnnotations(method, method2)) {
                                        ...
                                        // System.out.println("parameter success..............." + method.getName() + "
                                        // " + method2.getName() );
                                    }
                                }
                            }
    ...
    public static boolean checkParameterAnnotations(Method method, Method method2) {
        ...
        if (parameterAnnotations1.length == parameterAnnotations2.length
                && parameterAnnotations1[0].length == parameterAnnotations2[0].length) {
            for (int i = 0; i < parameterAnnotations1.length; i++) {
                for (int j = 0; j < parameterAnnotations1[i].length;) {
                    if (parameterAnnotations1[i][j].annotationType()
                            .equals(parameterAnnotations2[i][j].annotationType())) {
